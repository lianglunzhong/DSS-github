...
**/
	public function cookie($name='', $value='', $options='')
	{
		if($name)
		{
			if(is_array($options))
			{
				return setcookie($name, $value, $options['expire'], $options['path'], $options['domain'], $options['secure'], $options['httponly']);
			}
			else
			{
				return $value ? setcookie($name,$value) : setcookie($name, '', time()-3600);
			}
		}
		return $_COOKIE;
	}
